...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods) {
  if (action == GLFW_PRESS) {
    switch (key) {
      ...
      case GLFW_KEY_LEFT:
        if (keyboard_mode == 1)
          gl::Transform::left(amount, eye, up);
        else if (keyboard_mode == 0)
          tx -= amount * 0.01;
        break;
      case GLFW_KEY_UP:
        if (keyboard_mode == 1)
          gl::Transform::up(-amount, eye, up);
        else if (keyboard_mode == 0)
          ty += amount * 0.01;
        break;
      case GLFW_KEY_RIGHT:
  ...
  if (clicked_state) {
    ...
    if (x > current_vp_width) {
      if (previous_y_position - y > 0) {
        if (mouse_mode == 1)
          gl::Transform::up(-amount * sensitivity, eye, up);
      ...
      if (previous_x_position - x > 0) {
        if (mouse_mode == 1)
          gl::Transform::left(amount * sensitivity, eye, up);
        else if (mouse_mode == 0)
          tx -= amount * 0.005;
...
void display(float& ambient_slider, float& diffuse_slider, float& specular_slider, float& shininess_slider,
             bool custom_color, float& light_position, float& light_color) {
...
int main(int argc, char* argv[]) {
  ...
  if (gladLoadGL() == 0) {
    std::cout << "glad load opengl failed\n";
